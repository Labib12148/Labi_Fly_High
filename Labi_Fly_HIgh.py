...
class Bird:
    def __init__(self):
        ...
        self.velocity = 0
        self.images = [pygame.image.load("assets/imgs/Labi_Fly.png").convert_alpha(),
                       pygame.image.load("assets/imgs/Labi_Fly2.png").convert_alpha(),]
        self.current_image_index = 0
        self.image = self.images[self.current_image_index]
        # Rect without scaling
        self.rect = self.image.get_rect(topleft=(self.x, self.y))
        self.animation_tick = 0  # Counter to control animation speed
    ...
    def update(self):
        ...
        if self.animation_tick % 8 == 0:  # Change image every 10 ticks (adjust as needed)
            self.current_image_index = (self.current_image_index + 1) % len(self.images)
            self.image = self.images[self.current_image_index]

    def draw(self):
        # Scale the image to desired dimensions when drawing
...
def main():
    bird = Bird()
    pipes = [Pipe(SCREEN_WIDTH + i * 330) for i in range(5)]
        ...
        if state == INTRO:
            ...
            for pipe in pipes:
                if bird.rect.colliderect(pipe.rect_down) or bird.rect.colliderect(pipe.rect_up) \
                        or bird.rect.bottom <= 0 or bird.rect.top >= SCREEN_HEIGHT:
                    state = GAME_OVER
                elif not pipe.passed and bird.rect.x > pipe.rect_down.x:
                    pipe.passed = True
                    score += 1
